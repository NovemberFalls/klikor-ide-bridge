...
public final class RemoteActionServer implements AutoCloseable {
    ...
    public void start() throws IOException {
        ...
        log("Klikor IDE Bridge Remote Action Server listen on host " + String.join(",", LocalHostUtil.getLocalIPs()) + " port:" + server.getAddress().getPort());
    ...
    public void close() {
        server.stop(0);
        started = false;
        log("Stopped Klikor IDE Bridge Remote Action Server");
    }
    ...
    static class HandleHttpRequest implements HttpHandler {
        ...
        void responseError(HttpExchange exchange) throws IOException {
            ...
            String response = "404 - file not found from action server<br>" + "Your are visiting: " + path + "<br> Klikor IDE Bridge Http Server";
